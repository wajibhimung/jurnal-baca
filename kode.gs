...
function doPost(e) {
  try {
    ...
    switch (action) {
      ...
      case 'getStudentJournals': result = getStudentJournals(data.emailSiswa); break;
      ...
      case 'getLeaderboardData': result = getLeaderboardData(); break;
...
function login(email, password, role) {
  ...
  for (let i = 1; i < data.length; i++) {
    const row = data[i];
    const userEmail = (role === 'siswa') ? row[2] : row[0];
    const userPassword = (role === 'siswa') ? row[3] : row[2];
...
function getTeacherDashboardData() {
  ...
  const journals = ss.getSheetByName(SHEET_JURNAL).getDataRange().getValues().slice(1).map(r => ({
      ...
      namaSiswa: (students.find(s => s.idSiswa === r[1]) || {}).nama || 'N/A',
...
function getLeaderboardData() {
  ...
  jurnalData.forEach(row => {
    ...
    if (siswaMap[siswaId]) { siswaMap[siswaId].totalHalaman += halaman; siswaMap[siswaId].totalBuku += 1; }
  });
